...
public class SecurityConfig {
    ...
    @Value("${frontend.oauth-callback-path:/oauth/callback}")
    private String frontendOAuthCallbackPath;
    ...
    public CorsConfigurationSource corsConfigurationSource() {
        ...
        config.setAllowedOrigins(List.of("http://localhost:4200", "https://toxicbet.com.br"));
        config.setAllowedMethods(List.of("GET", "POST", "PUT", "DELETE", "OPTIONS", "PATCH"));
        config.setAllowedHeaders(List.of("Authorization", "Content-Type", "Accept", "publicId", "applicationPublicId", "applicationPublicID"));
    ...
    public AuthenticationSuccessHandler oAuth2AuthenticationSuccessHandler() {
        return (request, response, authentication) -> {
            ...
            String callbackUrl = frontendOrigin + frontendOAuthCallbackPath;

            String html = "<!DOCTYPE html>" +
                    "<html><body><script>" +
                    "const response = {" +
                    "  name: \"" + escapeJs(loginResponse.getUserName()) + "\"," +
                    "  email: \"" + escapeJs(loginResponse.getEmail()) + "\"," +
                    "  token: \"" + escapeJs(loginResponse.getToken()) + "\"," +
                    "  refreshToken: \"" + escapeJs(loginResponse.getRefreshToken()) + "\"" +
                    "};" +
                    "if (window.opener && !window.opener.closed) {" +
                    "  window.opener.postMessage({ type: 'google-login', payload: response }, \"" + escapeJs(frontendOrigin) + "\");" +
                    "  window.close();" +
                    "} else {" +
                    "  const encoded = encodeURIComponent(JSON.stringify(response));" +
                    "  window.location.replace(\"" + escapeJs(callbackUrl) + "?data=\" + encoded);" +
                    "}" +
